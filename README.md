...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create lammoc-platform-651f
  fly apps create lammoc-platform-651f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app lammoc-platform-651f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app lammoc-platform-651f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'lammoc-platform-651f-staging' does not match app name in config file 'lammoc-platform-651f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name lammoc-platform-651f-db
  fly postgres attach --app lammoc-platform-651f lammoc-platform-651f-db

  fly postgres create --name lammoc-platform-651f-staging-db
  fly postgres attach --app lammoc-platform-651f-staging lammoc-platform-651f-staging-db
  ```
